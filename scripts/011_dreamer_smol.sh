...
#SBATCH --job-name 011_dreamer_smol
...
uv run train-dreamer --logdir $logdir --configs mordorhike sizesmol --run.steps 4e5 --run.save_every 5e4  --run.eval_every 5e4  --seed $seed --run.train_ratio 512  --task mordorhike_hard --n_particles 1
end=`date +%s`
