...
#SBATCH --job-name 048_r2i_easy_no_translate_noise
...
echo "Training R2I"
uv run train-r2i --logdir $logdir --configs mordorhike --seed $seed --task "gymnasium_mordor-hike-easy-v0" --batch_size 4 --rssm.deter 1024 --rssm.units 1024 --rssm.hidden 512 --ssm.n_layers 4 --model_opt.lr 0.00017958212993107736 --actor_opt.lr 0.0002341129438718781 --critic_opt.lr 0.0002341129438718781 --.*\\.units 1024 --.*\\.mlp_units 1024 --.*\\.mlp_layers 1 --run.train_ratio 457 --env.kwargs "{'translate_std': 0.0}"

echo "Rolling out R2I"
uv run rollout-r2i --logdir $logdir

echo "Analyzing R2I"
uv run analyze --logdir $logdir --metric_dir "eval"
end=`date +%s`
