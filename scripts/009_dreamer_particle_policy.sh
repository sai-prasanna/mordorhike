...
#SBATCH --job-name 009_dreamer_particle_policy
...
if [ $SLURM_ARRAY_TASK_ID -eq 0 ]; then
    ...
    uv run eval-dreamer --logdir $logdir
    uv run eval-dreamer --logdir $logdir --mine_deep_set_size 256  --wandb.name"1337_big_ds"
elif [ $SLURM_ARRAY_TASK_ID -eq 2 ]; then
    logdir="experiments/mordor_hike/009_dreamer_particle_policy/13"
    uv run train-dreamer --logdir $logdir --configs mordorhike size2m --run.steps 5e5 --run.save_every 1e5  --run.eval_every 1e5  --seed 13 --run.train_ratio 512  --task mordorhike_hard --n_particles 4
    uv run eval-dreamer --logdir $logdir
    uv run eval-dreamer --logdir $logdir --mine_deep_set_size 256 --wandb.name"13_big_ds"
fi  
end=`date +%s`
