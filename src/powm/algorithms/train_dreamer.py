...
def make_env(config, env_id=0, estimate_belief=False, set_jax_flags=True):
    ...
    from powm.envs.mordor_hike_jax import MordorHikeJAX

    suite, task = config.task.split("_", 1)
    if suite == "mordorhike":
        task2cls = {
            "medium": MordorHikeJAX.medium,
            "hard": MordorHikeJAX.hard,
            "easy": MordorHikeJAX.easy,
        }
        env = task2cls[task](
...
def make_agent(config):
    env = make_env(config, estimate_belief=False, set_jax_flags=False)
...
def main(argv=None):
    ...
    train(
        ...
        bind(make_env, config, set_jax_flags=True, estimate_belief=False),
